...
//	fix32_shift( real, imag, M, -M/2 );
	...
	float avgdeviation = total_deviation/(double)(1<<M);
	printf( "Complex FFT RMS Error Squared Per Sample: %f\n", avgdeviation );

	if( avgdeviation > 20 )
	{
		fprintf( stderr, "Error: Real FFT error too high.\n" );
		exit( -55 );
	}
	...
	avgdeviation = total_deviation/(double)(1<<M);
	...
	if( nmatch + offset >= sizeof( convmapreal ) / sizeof( convmapreal[0] ) ) nmatch = sizeof( convmapreal ) / sizeof( convmapreal[0] ) - offset;
	...
	if( pwrPeak/pwrRMS < 3.5 )
	{
		fprintf( stderr, "Error: Correlation comparison bad.\n" );
		exit( -6 );
	}
